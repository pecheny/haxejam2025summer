...
class IntPlusTmpLabel extends Widget implements Updatable implements IStatLabel {
    ...
    var lbl:Label;
    var stat:TempIncGameStat<Float>;
    ...
    function setText(?_:Float) {
        var tmpVal = stat.tmp.value;
        var delta = if (tmpVal == 0) "" else if (tmpVal > 0) ' +${fmt(tmpVal)}' else ' -${fmt(tmpVal)}';
        lbl?.withText('$statName ${fmt(stat.prm.value)}$delta');
        t = duration;
    }

    function fmt(v:Float, prc:Int = 100) {
        return Std.int(v * prc) / prc;
    }
